...
module IF_stage(
	...
	//连接指令RAM
	output wire						inst_ram_c_en,//片选使能
	output wire						inst_ram_w_en,
	output wire						inst_ram_r_en,
	output wire[31:0]				inst_ram_addr,
	output wire[3:0]				inst_ram_byte_en,
	input  wire[31:0]				inst_ram_rdata,
	output wire[31:0]				inst_ram_wdata//实际上用不到指令RAM的写
    );
	wire[31:0]	PC_fromID;
	wire		sel_next_PC;
	...
	//控制流水线行为
	assign IF_ready_go=IF_ready_go_fromTR;
	...
	//指令RAM
	assign inst_ram_c_en=inst_ram_r_en | inst_ram_w_en;
	assign inst_ram_w_en=1'b0;
	assign inst_ram_r_en=IF_allow_in&Pre_to_IF_valid;//为什么是这个逻辑？
	assign inst_ram_addr=PC;//next_PC;
	assign inst_ram_wdata=32'bz;//不写指令RAM
	assign inst_ram_byte_en=4'b1111;
